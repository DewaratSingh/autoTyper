...
function handlePDFUpload(e) {
  ...
  showLoading("Loading PDF Slides...", "Reading file...");

  const reader = new FileReader();
  reader.onload = function (e) {
    updateLoadingProgress(20, "Converting slides...");
    setTimeout(() => {
      window.currentPDFBase64 = arrayBufferToBase64(e.target.result);
      updateLoadingProgress(40, "Processing PDF data...");
...
function processPDFData(arrayBuffer) {
  ...
  // Only change title if not already loading a project
  const titleEl = document.getElementById('loadingTitle');
  if (!titleEl || !titleEl.textContent.includes("Lecture")) {
    showLoading("Loading PDF Slides...", "Parsing PDF document...");
  }
  updateLoadingProgress(50, "Parsing PDF document...");
...
function renderPDFMarkers(pdf) {
  ...
  const titleEl = document.getElementById('loadingTitle');
  const loaderTitle = (titleEl && titleEl.textContent.includes("Lecture")) ? "Loading Lecture & PDF..." : "Loading PDF Slides...";
  showLoading(loaderTitle, `Generating previews (0/${totalPages})...`);
  updateLoadingProgress(50, `Generating previews (0/${totalPages})...`);
  ...
  for (let pageNum = 1; pageNum <= totalPages; pageNum++) {
    ...
    pdf.getPage(pageNum).then(function (page) {
      ...
      page.render({ canvasContext: context, viewport: viewport }).promise.then(function () {
        renderedPages++;
        const pct = 50 + Math.round((renderedPages / totalPages) * 50);
        updateLoadingProgress(pct, `Generating previews (${renderedPages}/${totalPages})...`);
...
function applyAccent(color, swatchEl) {
  ...
  if (swatchEl) {
    ...
    const swatches = document.querySelectorAll('.st-swatch');
    swatches.forEach(sw => {
