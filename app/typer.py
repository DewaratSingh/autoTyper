...
def _make_file_state():
    ...
    return {
        "cursor_x":          0,
        "cursor_y":          0,   # -1 = not yet positioned on first line
...
def process_tick():
    ...
    with lock:
        ...
        line_no      = st["lineNo"]
        prev_line_no = data[item["file"]][actual_step_idx - 1]["lineNo"] if actual_step_idx > 0 else -1    # -1 means we haven't typed anything yet in this file
